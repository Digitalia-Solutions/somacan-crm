...
export default function CategorySelectorField({ label, value, onChange, hint, multiple = false }) {
  ...
  const filtered = categories.filter((c) => {
    const name = c.name || c.title || c.nom || '';
    ...
    <div className="flex flex-col gap-1.5">
      ...
      <div className="border border-stone-200 rounded-xl bg-stone-50 max-h-48 overflow-y-auto">
        ...
        {!loading && !error && filtered.length === 0 && (
          <div className="py-3 px-3 text-sm text-stone-400">Aucune catégorie trouvée.</div>
        )}
        {!loading && filtered.map((category) => {
          const id = category._id || category.id;
          const name = category.name || category.title || category.nom || id;
          ...
          return (
            ...
            >
              ...
              {category.image && (
                <img src={category.image} alt="" className="w-6 h-6 object-cover rounded" />
              )}
